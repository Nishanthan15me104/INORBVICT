...
from pydantic_settings import BaseSettings # Required for defining the temp Settings class

# --- TEMPORARY HARDCODED CONFIGURATION ---
# This class and instance mirror the settings needed from config.py
# to eliminate the import error due to package structure issues.
class TemporarySettings(BaseSettings):
    RAG_CHUNK_SIZE: int = 256

settings_local = TemporarySettings()
# ----------------------------------------
...
def deduplicate_documents(
    ...
    for i, j, _ in duplicates:
        # Keep the document with more content (a simple heuristic)
...
def find_duplicates(
    ...
    threshold: float = 0.7,
    # Use the local settings object for num_perm
    num_perm: int = int(settings_local.RAG_CHUNK_SIZE * 0.5), 
